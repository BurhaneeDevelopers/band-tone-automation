...
import type { TranscribeState } from './useTranscribeYouTube';
...
export function useTranscribeFile() {
  ...
  const transcribe = useCallback(async (opts: FileTranscribeOptions) => {
    ...
    try {
      ...
      if (opts.segmentStart != null) formData.append('segment_start', String(opts.segmentStart));
      if (opts.segmentEnd != null) formData.append('segment_end', String(opts.segmentEnd));
      formData.append('melody_source', opts.melodySource ?? 'auto');
      formData.append('include_bass_for_low', String(opts.includeBassForLow ?? false));
